...
CREATE TABLE biz.interfaces (
  id              serial PRIMARY KEY,
  interface_id    text NOT NULL UNIQUE,     -- LAB-DEMO-MD-O001
  ...
  func_name       text NOT NULL UNIQUE,     -- lab_demo_md_a07_get_sample_type
  direction       text,                     -- Requester
  data_flow       text,                     -- O=出站
...
VALUES
  ('LAB-DEMO-MD-O001', 'DEMO', 'LAB', 'A.主数据同步（字典对照）', 'MD', 'A07',
   '检验样本类型下载', 'lab_demo_md_a07_get_sample_type', 'Requester', 'O', 'POST',
   '/api/demo/mdrs/v1/lis/samplesjf/getSampleType',
   'Requester下载样本类型字典用于项目对照')
